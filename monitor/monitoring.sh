...
sudo docker run  --name bottlenecks-barometer-server -tid --net=host \
  -v `pwd`/../src/collectd_sample_configs:/opt/collectd/etc/collectd.conf.d \
  -v ${MONITOR_CONFIG}/barometer_collectd.conf:/src/barometer/src/collectd/collectd/src/collectd.conf \
