...
def Get_Reply(test_config, task_id, time_test=1):
    reply_url = "http://%s/yardstick/results?task_id=%s" % (test_config['test_ip'], task_id)
...
def Send_Data(test_dict, test_config):
    base_url = ("http://%s/yardstick/testcases/%s/action"
               % (test_config['test_ip'], test_config['test_dir']))
...
def find_condition(con_dic):
    base_url = "http://%s/yardstick/asynctask?%s" % (con_dic['test_ip'].id)
    requests.post(
