...
function fn_vstf_test_config(){
    ...
    tester_testing_ip=`heat output-show ${STACK_NAME} tester_testing_ip`
    #`nova list | grep "vstf-tester" | grep "bottleneck-testing" | awk -F'bottleneck-testing=' '{print $2}' | awk '{print $1}'`
    target_testing_ip=`heat output-show ${STACK_NAME} target_testing_ip`
    #`nova list | grep "vstf-target" | grep "bottleneck-testing" | awk -F'bottleneck-testing=' '{print $2}' | awk '{print $1}'`
