opyright (c) 2018 Huawei Technologies Co.,Ltd and others.
...
wget https://storage.googleapis.com/golang/go${GOLANG_VERSION}.linux-amd64.tar.gz >${redirect}
...
echo "Adding golang env to ~/.profile"
GOROOT=/usr/local/go
GOPATH=${HOME}/go

if [[ $(cat ${HOME}/.profile | grep GOROOT) ]]; then
    echo "golang env alreay in ${HOME}/.profile"
else
