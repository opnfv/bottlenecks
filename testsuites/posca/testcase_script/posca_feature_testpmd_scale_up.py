...
cidr = "/home/opnfv/repos/yardstick/tests/vsperf/pvp_rfc2544_throughput_dpdk.yaml"
runner_DEBUG = False
...
def config_to_result(test_config, test_result):
    testdata = {}
    result = []
    test_result = result["result"]["testcase"]["pvp_rfc2544_throughput_dpdk"]["tc_data"]
    for result in len(test_result):
        testdata["vcpu"] = test_config["vcpu"]
        testdata["memory"] = test_config["memory"]
        testdata["nrFlows"] = test_result["data"]["nrFlows"]
        testdata["packet_size"] = test_result["data"]["packet_size"]
        testdata["throughput"] = test_result["data"]["throughput_rx_mbps"]
...
def do_test(test_config, Use_Dashboard, context_conf):
    ...
    save_data = config_to_result(test_config, data['result'])
...
def run(test_config):
    ...
    LOG.info("The max data is %d", data_reply["throughput"])
    return data_reply
