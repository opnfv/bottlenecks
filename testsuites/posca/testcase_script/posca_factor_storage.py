...
def env_pre(con_dic):
    ...
    storperf_usage.storperf_env_prepare(con_dic, con_dic['load_manager']['agent_count'], con_dic['load_manager']['agent_image'], con_dic['load_manager']['agent_flavor'], con_dic['load_manager']['volume_size'])
...
def run(test_config):
    ...
    data = {}
    data["block_sizes"] = conf_parser.str_to_list(con_dic['scenarios']['block_sizes'])
    data["workload"] = conf_parser.str_to_list(con_dic['scenarios']['workload'])
    data["queue_depths"] = conf_parser.str_to_list(con_dic['scenarios']['queue_depths'])
    ...
    for test_x in data["workload"]:
        data_max["throughput"] = 1
        bandwidth_tmp = 1
        for test_y in data["block_sizes"]:
            for test_z in data["queue_depths"]:
                case_config = {
